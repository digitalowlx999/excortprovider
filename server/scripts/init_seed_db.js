...
async function run() {
  // Use absolute path for dotenv to ensure it loads from the root correctly
  ...
  try {
    ...
    for (const [category, urls] of Object.entries(IMAGES)) {
      for (const url of urls) {
        // Must use array structure for parameters properly
        await pool.query('INSERT INTO seed_images (category, url) VALUES (?, ?)', [category, url]);
        count++;
      }
    }
    console.log(`SUCCESS: Inserted ${count} images into seed_images table.`);
  } catch(e) {
    console.error("DEBUG ERROR CODE:", e.code);
    console.error("DEBUG ERROR MSG:", e.sqlMessage || e.message);
  } finally {
    await pool.end();
    process.exit(0);
  }
